...
<link
	...
	rel="stylesheet">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/admincss/icomoon/style.css">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/admincss/owl.carousel.min.css">
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/admincss/style.css">
		...
		<div class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mt-3 pb-3">
			<p id="keymap">
				<i class="bi bi-building"
					style="font-weight: bold; font-size: 16px;"></i></span>&nbsp;&nbsp;&gt;&nbsp;&nbsp;<span
					data-feather="home"></span> &nbsp;<strong style="font-size: 13px;">고객문의
					관리</strong>
			</p>
			<!-- 조건설정하는 헤드부분 -->
							...
							<form action="modify" method="post">
								...
								<table class="table">
									<tbody>
										...
										<tr>
											<td>내용</td>
											<td style="padding: 10px;" height="200">${inquiry.content}</td>
										</tr>
										<c:if test="${inquiry.answerState eq 'Y' }">
											<tr>
												<td>답변날짜</td>
												<td><fmt:formatDate
														value="${inquiry.answerCreatedDate}" pattern="yyyy-M-d" /></td>
											</tr>

											<tr>
												<td><label class="form-label">답변내용</label></td>
												<td colspan="3" style="padding: 10px;" height="200"><textarea
														class="form-control" style="width: 600px;"
														name="answerContent">${inquiry.answerContent}</textarea></td>
											</tr>
										</c:if>
									</tbody>
								</table>
								<c:if test="${inquiry.answerState eq 'N'}">
									<button type="button" class="btn btn-inquiry"
										onclick="location.href='/admin/answerForm?no=${inquiry.no}'">답변작성</button>
								</c:if>
								<button type="submit" class="btn btn-inquiry">답변등록</button>
								<button type="button" class="btn btn-inquiry"
									onclick="location.href='/admin/delete?no=${inquiry.no}'">답변내역
									삭제</button>
								<button style="float: right" type="button"
									class="btn btn-inquiry"
									onclick="location.href='/admin/inquiry'">목록</button>
								</div>
								</div>
								</div>
							</form>
