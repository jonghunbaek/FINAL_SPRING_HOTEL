			...
			<div class="col">
				<div class="table-responsive mt-1">
					<table class="table custom-table">
						...
						<tbody id="body">
							<c:forEach var="roomRev" items="${diningRevs }">
								<tr id="${diningRev.no }">
									<th scope="row">
										<label class="form-check-label" for="flexCheckDefault">
											<input type="checkbox" name="checkedNo" value="${diningRev.no }"style="accent-color: RGBa(255, 56, 92, 0.1); zoom:1.3;">
										</label>
									</th>
									<td>${diningRev.no }</td>
									<td><a href="">${diningRev.user.name }</a><small class="d-block">${diningRev.user.tel }</small></td>
									<td>${diningRev.dn.location.name }</td>				
									<td>${diningRev.dn.no }(${diningRev.roomCategory.name })</td>
									<c:choose>
										<c:when test="${diningRev.status eq 'O'}">
											<td style="color:RGBa(255, 56, 92, 0.7); font-weight:bold;">임박</td>
										</c:when>	
										<c:when test="${diningRev.status eq 'I'}">
											<td style="color:RGBA(0, 0, 255, 0.6); font-weight:bold;">체크인</td>
										</c:when>
										<c:when test="${diningRev.status eq 'R'}">
											<td style="color:RGBa(0, 128, 128, 0.7); font-weight:bold;">예약중</td>
										</c:when>
										<c:when test="${diningRev.status eq 'D'}">
											<td>예약취소</td>
										</c:when>							
									</c:choose>						
									<td><fmt:formatDate value="${diningRev.checkinTime }" pattern="yyyy-MM-dd" /></td>
									<td><button type="button" class="btn btn-table" id="btn-table-${diningRev.no }" data-tag="${diningRev.no }"><i class="bi bi-box-arrow-in-right"></i></button></td>
								</tr>
							</c:forEach> 
						</tbody>
					</table>
        ...
        <form id="detail-modal-form" method="post">
          <div class="row">
	          ...
	          <div class="col-3 mb-3">
		        ...
		        <input type="text" class="form-control" id="dnNo-modal" disabled>
	          </div>
	          <div class="col-4 mb-3">
          ...
          <div class="row">
	          ...
	          <div class="col-4 mb-3">
	            <div class="form-check">
				  <input class="form-check-input" type="checkbox" name="allergy" value="1" id="meat">
				  <label class="form-check-label" for="meat">육류</label>
				  <input class="form-check-input" type="checkbox" name="allergy" value="2" id="seafood">
				  <label class="form-check-label" for="seafood">어패류</label>
				  <input class="form-check-input" type="checkbox" name="allergy" value="3" id="milk">
				  <label class="form-check-label" for="milk">유제품</label>
				  <input class="form-check-input" type="checkbox" name="allergy" value="4" id="nut">
				  <label class="form-check-label" for="nut">견과류</label>
				</div>
	          </div>
	          <div class="col-4 mb-3">
